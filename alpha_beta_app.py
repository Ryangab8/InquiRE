...
def fetch_raw_data_multiple(msa_ids, start_ym, end_ym):
    ...
    start_dt = datetime.datetime(sy, sm, 1)
    end_dt   = datetime.datetime(ey, em, 1)
    return df_full[
...
def compute_rolling_alpha_beta_time_series(df_raw_ts, start_ym_ts, end_ym_ts):
    ...
    # Drop rows that are fully NaN, then sort by date so rolling windows work
